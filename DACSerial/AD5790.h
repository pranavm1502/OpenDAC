...
class AD5790
{
    public:
        AD5790(int sync);
        void initialize_DAC(int pin, int _sync);
        
        long set_value(int _sync, long bitcode);
        void set_gcurrval(long input);
        void ramp(int _sync, long bitcode, long step_size, int delta_t);
        long read_CTRL_register();
        long read_DAC_register(int _sync);
